...
MODES = {"train", "valid", "predict", "export", "track", "benchmark"}
...
CLI_HELP_MSG = f"""
    ...
    3. Val a pretrained detection model at batch-size 1 and image size 640:
        yolo valid model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640
...
def merge_equals_args(args: List[str]) -> List[str]:
    ...
    1. ['arg', '=', 'valid'] becomes ['arg=valid']
    2. ['arg=', 'valid'] becomes ['arg=valid']
    3. ['arg', '=valid'] becomes ['arg=valid']
    ...
    while i < len(args):
        ...
        if arg == "=" and 0 < i < len(args) - 1:  # merge ['arg', '=', 'valid']
            new_args[-1] += f"={args[i + 1]}"
            i += 2
            continue
        elif arg.endswith("=") and i < len(args) - 1 and "=" not in args[i + 1]:  # merge ['arg=', 'valid']
            new_args.append(f"{arg}{args[i + 1]}")
            i += 2
            continue
        elif arg.startswith("=") and i > 0:  # merge ['arg', '=valid']
            new_args[-1] += arg
            i += 1
            continue
...
def entrypoint(debug=""):
    ...
    Examples:
        ...
        >>> entrypoint("valid model=yolo11n.pt data=coco8.yaml batch=1 imgsz=640")
