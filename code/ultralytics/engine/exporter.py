    ...
    def __call__(self, model=None) -> str:
        ...
        if any(f):
            ...
            s = (
                ""
                if square
                else f"WARNING ⚠️ non-PyTorch valid requires square images, 'imgsz={self.imgsz}' will not "
                f"work. Use export 'imgsz={max(self.imgsz)}' if valid is required."
            )
            ...
            LOGGER.info(
                ...
                f'\nValidate:        yolo valid task={model.task} model={f} imgsz={imgsz} data={data} {q} {s}'
