...
class Model(nn.Module):
    ...
    Methods:
        ...
        valid: Validates the model on a dataset.
    ...
    def _check_is_pytorch_model(self) -> None:
        ...
        if not (pt_module or pt_str):
            raise TypeError(
                ...
                f"PyTorch models can train, valid, predict and export, i.e. 'model.train(data=...)', but exported "
                f"formats like ONNX, TensorRT etc. only support 'predict' and 'valid' modes, "
    ...
    def val(
        ...
        **kwargs,
        ...
        Examples:
            >>> model = YOLO("yolo11n.pt")
            >>> results = model.valid(data="coco8.yaml", imgsz=640)
            >>> print(results.box.map)  # Print mAP50-95
        """
        custom = {"rect": True}  # method defaults
        args = {**self.overrides, **custom, **kwargs, "mode": "valid"}  # highest priority args on the right
